...
def gen_random_multiagent_seperable_reward_table(agents: List[Agent]):
    g_vals = np.random.uniform(low=0, high=1, size=tuple([len(agent.action_space) for agent in agents]))
    u_list = []

    for i, agent in enumerate(agents):
        u_i_vals = np.random.uniform(low=0, high=1, size=(len(agent.state_space), len(agent.action_space)))
        u_list.append(u_i_vals)
    return g_vals, u_list
...
if __name__ == "__main__":
    ...
    start_state = 0
    mdp = MDP(states, actions, transition_prob, start_state)

    # Initialize agents
    agents: List[Agent] = []
    for i in range(n_agents):
        agents.append(Agent(id=i+1, mdp=copy.deepcopy(mdp)))

    n_sim = 300
    for i_sim in range(n_sim):
        g_vals, u_list = gen_random_multiagent_seperable_reward_table(agents)


        multi_agent = MultiAgent(agents=agents,
                                 multi_agent_reward=SeperableMultiAgentReward(num_agents=len(agents),
                                                                              joint_static_term=g_vals,
        ...
        # array([23.375417, 23.63725427, 24.08559862, 24.3474359])
        # Run simulation
        n_steps = 100
        ...
        #
        # policy_graph = simulation.multi_agent.build_policies_best_response_graph()
        # simulation.multi_agent.plot_policy_best_response_graph(policy_graph)
        #
